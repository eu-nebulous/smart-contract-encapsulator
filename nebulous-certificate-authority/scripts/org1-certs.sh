...
CA_SERVICE_NAME="${CA_SERVICE_NAME:-ca-org1-resourceprovider}"
CA_PORT="${CA_PORT:-7057}"
CA_URL="${CA_SERVICE_NAME}:${CA_PORT}"

# CA logical name used in fabric-ca-client --caname
CA_CANAME="${CA_CANAME:-ca-org1-resourceprovider}"
CA_ADMIN_USER="${CA_ADMIN_USER:-ca-admin-${CA_CANAME}}"

# Secrets mounted in container (Swarm best practice)
CA_ADMIN_SECRET_FILE="${CA_ADMIN_SECRET_FILE:-/run/secrets/ca_org1_bootstrap_pw}"
PEER_PW_FILE="${PEER_PW_FILE:-/run/secrets/ca_org1_bootstrap_pw}"
USER_PW_FILE="${USER_PW_FILE:-/run/secrets/ca_org1_bootstrap_pw}"
ADMIN_PW_FILE="${ADMIN_PW_FILE:-/run/secrets/ca_org1_bootstrap_pw}"

# Files & directories
ROOT_DIR="${ROOT_DIR:-/workspace}"
FABRIC_CA_CLIENT_HOME="${FABRIC_CA_CLIENT_HOME:-${ROOT_DIR}/organizations/peerOrganizations/resourceproviderorg.neb.com/ca-client}"
TLS_CERT_FILE="${TLS_CERT_FILE:-${ROOT_DIR}/organizations/fabric-ca/org1-resourceprovider/tls-cert.pem}"

# Peer configuration
PEER_NAME="${PEER_NAME:-peer0}"
PEER_DOMAIN="${PEER_DOMAIN:-resourceproviderorg.neb.com}"
...
echo "Org1 (resourceprovider) certificates setup completed successfully."